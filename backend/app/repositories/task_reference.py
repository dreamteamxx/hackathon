...
from app.schemas import TaskReferenceUpdate, TaskReferenceRead

logger = logging.getLogger(__name__)


class TaskReferenceRepo(SQLAlchemyRepo):
    async def get_tasks_references(self) -> list[TaskReferenceRead] | None:
        stmt = await self.session.scalars(
            select(models.TasksReference).order_by(models.TasksReference.priority)
        )
        result = stmt.all()
        return list(map(models.TasksReference.to_dto, result)) if result else None

    async def create_task_reference(
        self, task_reference: TasksReference
    ) -> TasksReference:
        try:
    ...
    async def update_task_reference(
        self, task_reference_id, task_reference: TaskReferenceUpdate
    ) -> TaskReferenceUpdate:
        stmt = (
            update(TasksReference)
            .where(TasksReference.id == task_reference_id)
            .values(**task_reference.model_dump())
        )
    ...
    async def delete_task_reference(self, task_reference_id: int) -> None:
        stmt = delete(models.TasksReference).where(
            models.TasksReference.id == task_reference_id
        )
        try:
            await self.session.execute(stmt)
            await self.session.commit()
